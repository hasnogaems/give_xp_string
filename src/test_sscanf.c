...
START_TEST(s_check) {
  ...
  float f1 = 0.0;
  float f2 = 0.0;
  s21_sscanf("656.1e+10", "%e", &f1);
  sscanf("656.1e+10", "%e", &f2);
  ck_assert_float_eq(f1, f2);
  s21_sscanf("1.1", "%E", &f1);
  sscanf("1.1", "%E", &f2);
  ck_assert_float_eq(f1, f2);
  s21_sscanf("1.1", "%ff", &f1);
  sscanf("1.1", "%f", &f2);
  ck_assert_float_eq(f1, f2);
  s21_sscanf("1.1", "%g", &f1);
  sscanf("1.1", "%g", &f2);
  ...
  unsigned int x1 = 0x0;
  unsigned int x2 = 0x0;
  sscanf("0x10bb4cbb4", "%x", &x1);
  s21_sscanf("0x10bb4cbb4", "%x", &x2);
  ck_assert_int_eq(x1, x2);
  sscanf("0x10bb4cbb4", "%X", &x1);
  s21_sscanf("0x10bb4cbb4", "%X", &x2);
  ck_assert_int_eq(x1, x2);
  sscanf("3750", "%o", &x1);
  s21_sscanf("3750", "%o", &x2);
  ck_assert_int_eq(x1, x2);
  sscanf("02024", "%i", &x1);
  s21_sscanf("02024", "%i", &x2);
  ck_assert_int_eq(x1, x2);

  void *ptr1 = 0x0;
  void *ptr2 = 0x0;

  sscanf("0x2024", "%p", &ptr1);
  s21_sscanf("0x2024", "%p", &ptr2);
  ...
  char str5[18] = {0};
  char str6[18] = {0};
  s21_sscanf("    hello  world!", "%s%n%s", str5, &n1, str2);
  sscanf("    hello  world!", "%s%n%s", str6, &n2, str2);
  ck_assert_int_eq(n1, n2);
}
END_TEST

START_TEST(all_check) {
  char str1[7] = {0};
  char str2[7] = {0};
  ...
  result1 = s21_sscanf("string 6.1e+1c 20    0x2024        0x2024 -120 0256",
                       "%s%e%n%c%u%i%p%d%o%n", str1, &f1, &n1, &c1, &u1, &x1,
                       &ptr1, &a1, &x3, &n3);
  result2 = sscanf("string 6.1e+1c 20    0x2024        0x2024 -120 0256",
                   "%s%e%n%c%u%i%p%d%o%n", str2, &f2, &n2, &c2, &u2, &x2, &ptr2,
                   &a2, &x4, &n4);
...
START_TEST(length_check) {
  long double f1 = 0.0;
  long double f2 = 0.0;

  short int x1 = 0x0;
  short int x2 = 0x0;
  unsigned long int x3 = 0x0;
  unsigned long int x4 = 0x0;
  ...
  int result1 = 0;
  int result2 = 0;
  result1 = s21_sscanf("6.1e+1 20    0x2024        0x2024 -120 0256",
                       "%Le%20hu%6hi%p%hd%3lo%n", &f1, &u1, &x1, &ptr1, &a1,
                       &x3, &n3);
  result2 =
      sscanf("6.1e+1 20    0x2024        0x2024 -120 0256",
             "%Le%20hu%6hi%p%hd%3lo%n", &f2, &u2, &x2, &ptr2, &a2, &x4, &n4);
...
START_TEST(unsigned_check) {
  ...
  int result1 = 0;
  int result2 = 0;
  result1 = s21_sscanf("-20 -1 -0256 -120 -0x24 -0x24  0x1", "%i%d%o%u%x%X%p",
                       &i1, &d1, &o1, &u1, &x1, &X1, &ptr1);
  result2 = sscanf("-20 -1 -0256 -120 -0x24 -0x24  0x1", "%i%d%o%u%x%X%p", &i2,
                   &d2, &o2, &u2, &x2, &X2, &ptr2);
...
END_TEST
START_TEST(ptr_check) {
  void *ptr1 = 0x0;
  void *ptr2 = 0x0;

  s21_sscanf("-0x1", "%p", &ptr1);
  sscanf("-0x1", "%p", &ptr2);

  ck_assert_ptr_eq(ptr1, ptr2);
}
END_TEST
START_TEST(format_check) {
  int a1 = 0;
  int a2 = 0;
  int a4 = 0;
  int a3 = 0;
...
START_TEST(floats_sci1) {
  ...
  ck_assert_int_eq(res1, res2);
  ck_assert_double_eq(a1, a2);
  ck_assert_double_eq_tol(b1, b2, 10e-6);
  ck_assert_double_eq(c1, c2);
  ck_assert_double_eq_tol(b1, b2, 10e-6);
}
END_TEST
...
START_TEST(floats_sci2) {
  ...
  ck_assert_double_eq_tol(d1, d2, 10e-6);
}
END_TEST
...
START_TEST(floats_sci5) {
  ...
  ck_assert_ldouble_eq(c1, c2);
  ck_assert_ldouble_eq_tol(d1, d2, 10e-6);
}
END_TEST
...
Suite *suite_sscanf(void) {
  ...
  tcase_add_test(tc, s_check);
  tcase_add_test(tc, all_check);
  tcase_add_test(tc, asterisk_check);
  tcase_add_test(tc, width_check_);
  tcase_add_test(tc, length_check);
  tcase_add_test(tc, unsigned_check);
  tcase_add_test(tc, ptr_check);
  tcase_add_test(tc, format_check);
