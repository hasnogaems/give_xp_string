...
void scanf_write_string(flagscanf* Flags, va_list arg, const char** source) {
  ...
  if (!Flags->asterisk) {
    ...
    while (**source != ' ' && **source != '\0' && width_check(*Flags) &&
           **source != '\t') {  //пишем из source в буфер, а почему нельзя сразу
                                //писать в variable?
...
void scanf_write_decimal_octal_hex(const char** source, flagscanf* Flags,
  ...
  if (**source == ' ' || (**source >= 0 && **source <= 57 && **source != 32)) {
    Flags->failed = 0;
    if (**source != '\0' && **source != ' ' &&
        (is_octal ? **source < '8' : 1)) {  //вообще ведь нельзя вставить if в
                                            // while условие? только через ?
      if (**source >= 0 && **source <= 57 && is_int_f(**source)) {
        if (**source == '0' &&
            *(*source + 1) == 'x') {  // priority of * highter than + but not
                                      // highter than ++ seems like
          is_hex = 1;
          (*source) = (*source) + 2;
        }
        if (**source == '0' && is_int_f(*(*source) + 1) && !is_hex) {
          // if(**source=='0'&&is_int_f(&((*source)+1))){ почему так нельзя
          // хотел передавать функции указатель на указатель, переделал функцию
          // чтобы брала чар
          is_octal = 1;
          (*source)++;
        }
        if (!is_hex && !is_octal)
          is_int = 1;  //пишем в variable только если флаг поднят, если я сделаю
                       // int is_int прямо сдесь это плохо, это значит будет
                       //переинициализация каждый цикл или норм и оно не будет
                       //нагружать программу и инициализирует только 1 раз?
        while (!Flags->base.x && **source != ' ' && **source != '\0' &&
               (is_octal ? **source < '8' : 1) &&
               (is_hex ? hex_check(source) : 1) &&
               (is_int ? is_int_f(**source) : 1) && width_check(*Flags) &&
               !((**source == 43 || **source == 45) &&
                 !start)) {  //если не поставить последнее условие в скобки то
                             //оно считает все до ? условием, и оно никогда не
                             //выполняется, следовательно всегда 1
          *pbuffer = **source;
          start = 0;
  ...
  //     if(Flags->base.x){
  //     if(**source=='
  //     '||(**source>=0&&**source<=57&&**source!=32)||(**source>=65&&**source<=70)||(**source>=97&&**source<=102)){
  // *result=**source;
  // Flags->failed=0;
  // (*source)++;
  //     }}
  while (**source == ' ' || **source == '\n') {
    (*source)++;
  }  // skip spaces and newlines for %x border case
  buffer_integer = atoi(buffer);
  while (**source != '\0' && Flags->base.x &&
         ((**source >= 0 && **source <= 57 && **source != 32) ||
          (**source >= 65 && **source <= 70) ||
          (**source >= 97 && **source <= 102)) &&
         width_check(*Flags)) {
    Flags->failed = 0;
    *pbuffer = **source;
...
void data_type_for_i(long long int* result, va_list arg, flagscanf* Flags) {
  if (Flags->failed == 0) {
    ...
    } else if ((!Flags->l && !Flags->ll) && (Flags->base.is_unsigned == 1) &&
               Flags->base.is_ptr == 1) {
...
void scanf_write_int(flagscanf* Flags, const char** source,
                     long long int* result, va_list arg) {
  Flags->failed = 1;
  // int* i=va_arg(arg, int*);// какое будет поведение у  va_arg  если тип
  // аргумента не соответствует, например мы указываем int* а там лежит  char*
  ...
  if (**source != '\0' && (**source >= 0 && **source <= 57 && **source != 32) &&
      !double_sign_check(source,
                         Flags)) {  //это нужно чтобы не двигало курсор если мы
                                    //пытаемся прочитать строку как d
    while (**source != '\0' && **source != ' ') {
      if (**source >= 0 && **source <= 57 && **source != 32) {
        is_int = 1;
        Flags->failed = 0;
        while (**source != ' ' && **source != '\0' && width_check(*Flags) &&
               !((**source == 43 || **source == 45) && !start) &&
               is_int_f(**source)) {
          *pbuffer = **source;
          start = 0;
          pbuffer++;
        ...
        *(pbuffer) = '\0';
        ...
        i_i =
            atoi(buffer);  // buffer почему-то остается в памяти, поэтому
                           // заводим счетчик count и зануляем buffer после atoi
        while (count > -1) {
          buffer[count] = '\0';
          count--;
        }
...
void sscanf_write_o(va_list arg, const char** source, flagscanf* Flags) {
  Flags->failed = 1;
  long long int* variable_adress = va_arg(
      arg, long long int*);  //тут был просто int и поэтому в тестах ломалось
                             // where a1 holds the expected value 94862942690972
                             //(octal 57234 interpreted as a 64-bit unsigned
                             // integer), and a2 holds the value 24220 (octal
                             // 57234 interpreted as an unsigned long int, which
                             // is likely 32 bits on the test environment).
    ...
    while (**source != '\0' && **source != ' ' && is_int_f(**source) &&
      ...
      if (**source >= 0 && **source <= 57 && is_int_f(**source)) {
        if (**source == '0' && is_int_f(*(*source) + 1) && !is_hex) {
          ...
          // хотел передавать функции указатель на указатель, переделал функцию
          // чтобы брала чар
          is_octal = 1;  // width-- to compensate for skipped 0, actually just
                         // removing (*source)++ fixed 2 checks
        }
        if (!is_hex && !is_octal)
          is_int = 1;  //пишем в variable только если флаг поднят, если я сделаю
                       // int is_int прямо сдесь это плохо, это значит будет
        ...
        *pbuffer = '\0';
        //тут была сега если вставить отмотку сюда, то идет бесконечный цикл и
        // pbuffer растет вплоть до segfault
      }
    }
...
void sscanf_write_e(va_list arg, const char** source, flagscanf* Flags) {
  ...
  if (**source >= 0 && **source <= 57 && **source != 32 && **source != 46) {
    ...
    if (**source != '\0' && **source != ' ') {
      ...
      for (int i = 1; *(*source + i) != 32 && *(*source + i) != '\0' &&
                      !type.is_scientific;
           i++) {  //тут если вместо 32 поставить ' ' почему то выкидывает из
                   //цикла на одну итерацию позже
        if (*(*source + i) == 'e' || *(*source + i) == 'E') {
          type.is_scientific = 1;  // printf("is scientific\n");
        }
      }
      if (!type.is_hex && !type.is_octal && !type.is_scientific)
        type.is_int = 1;
      while (**source != 32 && **source != '\0' && width_check(*Flags) &&
             complex_while_check(source)) {
        *pbuffer = **source;
        (*source)++;
        pbuffer++;
        if (Flags->width != -1) Flags->width--;
...
int complex_while_check(const char** source) {
  ...
  if (**source != ' ' &&
      ((is_int_f(**source) ||
        ((is_int_f(*(*source - 1))) && (**source == 'e' || **source == 'E')))))
    is_true = 1;
...
long double scientific_to_float(char* string) {
  char pre_dot[100000] = {
      '\0'};  //тут был stack smashing если маленькие размеры массивов
...
long double exponent_f(
    char exp[],
    long double pre_plus_post) {  //тут тоже была ошибка точности если
                                  //передавали prepluspost как float
  int is_negative = 0;
  float scale = 0.0;
...
long double a_to_float(char* string) {
  ...
  long double return_this =
      (pre_dot_float + post_dot_float);  //тут если return_this type float то
                                         //начинают лезть микропогрешности
...
int s21_sscanf(const char* source, const char* format, ...) {
  ...
  while (*format != '\0' && *source != '\0' &&
         !Flagscanf
              .failed) {  // weird if I remove source check then hex_empty fails
    // printf("here?\n");
    while (*format == ' ') {
      format++;
      while (*source == ' ') {
    ...
    while (*format == '\t') format++;
    if (*format != '%' && !Flagscanf.failed)
      Flagscanf.failed = 1;  //чиним последний тест
    ...
    if (*format == '%' && Flagscanf.failed == 0) {
      ...
      if (*format == '%') {
        ...
        format++;
        Flagscanf.failed = 1;
      }  // fixed format_check last test not counting writted correctly, also it
         // is implementation of %%

      //  printf("test");
      // Flagscanf=scanfparser_flags(&format); // заполняем от ' ' до 0 почему
      // не растет указатель я разименовываю 1 раз, значит должен расти формат
      scanfparser_spec(&format,
                       &Flagscanf);  // заполняем спецификаторы например d или s
      if (Flagscanf.base.n) {
        if (!Flagscanf.asterisk) *(va_arg(args, int*)) = source - start;
      }
      // Flagscanf=scanfparser(format);
      scanf_concat_type(&Flagscanf, args,
                        &source);  //возвращаем то, что мы пишем в переменную,
      // va_arg(args,char*);

      //как вообще подставить в функцию имя переменной когда оно само
      //переменная? strcat(, tmp); move_str=strlen(tmp);
    }
...
void scanfparser_spec(const char** format, flagscanf* Flags) {
  ...
  while (**format != '\0' && **format != '%' && **format != ' ' &&
         **format != '\t' && !Flags->base.format_received) {
    // printf("here?:82");
    switch (**format) {
        // case'[':
...
flags parser(const char** format, flags Flags) {
  ...
  while (**format == 'd' || **format == 's' || **format == 'i' ||
         **format == 'e' || **format == 'o' || **format == 'E' ||
         **format == 'g' || **format == 'G' || **format == 'f' ||
         **format == 'p' || **format == 'u' || **format == 'c' ||
         **format == 'n' || **format == 'x' || **format == 'X') {
